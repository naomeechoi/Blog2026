...
실제 객체 크기는 그대로 두고, 참조 타입을 부모의 타입로 변환해 실제 접근 가능한 메모리 공간을 제한한다. 따라서 업케스팅은 안전하다.
다운캐스팅은 안전하지 않은데 예를 들어 parent 객체를 child로 보는 순간, child 고유 멤버에 접근하면 정의되지 않은 동작이 발생하기 때문이다.
다운 캐스팅은 안전하지 않으므로 최대한 쓰지 않는게 좋다.
보통 자식 클래스를 부모 클래스로 치환하다가 다시 자기 형태로 돌아올 때 사용한다.
다이나믹 캐스트는 실제 객체 타입을 확인하여, 페어런츠가 실제로 차일드 객체를 가리키는지 확인하여 캐스팅한다.
