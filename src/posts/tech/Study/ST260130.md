...
### 1) 헤더에 정의가 있고, 여러 cpp에서 이것을 사용하면 링크단에서 문제가 생긴다.
  - Util.h:
   ```
#pragma once
...
### 2) 렌더링 로직을 만들 때 참고할 수 있는 구조
엑터: 렌더러 컴포넌트 소유, 렌더러 컴포넌트에게 어떻게 그려야 한다고 전달
렌더러 컴포넌트: 전달받은 정보를 기반으로 렌더러 커맨드 만든다.
렌더러 커맨드: 어떤 텍스처를 어디에 그릴지 같은 실제 그리기 정보를 담는다.
렌더러: 렌더러 커맨드 벡터 순회하며 그래픽 API 호출한다.
